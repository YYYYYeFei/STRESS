...
def local_sampling(x, group_size, unfold_dict, output=0, tp='bhwc'):
    ...
    # print('local sample!')
    # print('x.shape:',x.shape)#[4, 180, 32, 32]
    # print('**unfold_dict:', unfold_dict)#{'kernel_size': (16, 16), 'stride': (16, 16), 'padding': (0, 0)}
    # print('F.unfold:', F.unfold(x, **unfold_dict).shape)#[4, 46080, 4]
...
def im3col(img,kernel_size, stride=1):
    """
    Parameters
    ----------
    ...
    out_d = (D - kernel_d) // stride_d + 1
    out_h = (H - kernel_h) // stride_d + 1
    out_w = (W - kernel_w) // stride_d + 1

    col = torch.zeros((N, C, kernel_d, kernel_h, kernel_w, out_d, out_h, out_w), device=img.device)
    ...
    col = col.permute(0,5,6,7, 1, 2, 3,4).contiguous().reshape(N, -1,out_d* out_h * out_w)
    # print(col.shape)
    return col